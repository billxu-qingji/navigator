...
+function () {
	...
	function Bubble() {
		...
		for (var i = 0; i < 4; i++) {
			...
			if (i == 3) {
				if (_IE6_)
					sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=public/img/heart.png)";
				else
					sty.backgroundImage = "url(public/img/user.png)";
				break;
			}
			...
			if (STD) {
				sty.backgroundImage = "url(public/img/ch" + i + ".png)";
				arrFlt[i] = sty;
			}
			else {
				sty.filter = "alpha progid:DXImageTransform.Microsoft.AlphaImageLoader(src=public/img/ch" + i + ".png)";
